...
class BottleneckB(nn.Module):
    ...
    def forward(self, x):
        ...
        # downsample always is None, because stride=1 and inplanes=expansion * planes
        if self.downsample is not None:  
            residual = self.downsample(x)
...
class DetNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        """
        Args:
            layers: block numbers of each layer
        """
        self.inplanes = 64
        ...
        # conv4.x,conv5.x,conv6.x
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2)
        # from now on, bock is different from Bottleneck
